...
const QR_VERSIONINFO QR_VersonInfo[] = {{0}, // Ver.
...
int m_nSymbleSize;																						//真实数据大小
unsigned char	 m_byModuleData[MAX_MODULESIZE][MAX_MODULESIZE];//用来存储二维码数据
...
char funtion(int ncRSCodeWord,int j)
{
	...
	switch(ncRSCodeWord)
	{
	case 10:res=byRSExp10[j]; break;
	case 13:res=byRSExp13[j]; break;
	case 15:res=byRSExp15[j]; break;
	case 16:res=byRSExp16[j]; break;
	case 17:res=byRSExp17[j]; break;
	case 18:res=byRSExp18[j]; break;
	case 20:res=byRSExp20[j]; break;
	case 22:res=byRSExp22[j]; break;
	case 24:res=byRSExp24[j]; break;
	case 26:res=byRSExp26[j]; break;
	case 28:res=byRSExp28[j]; break;
	case 30:res=byRSExp30[j]; break;
	case 32:res=byRSExp32[j]; break;
	case 34:res=byRSExp34[j]; break;
	case 36:res=byRSExp36[j]; break;
	case 38:res=byRSExp38[j]; break;
	case 40:res=byRSExp40[j]; break;
	case 42:res=byRSExp42[j]; break;
	case 44:res=byRSExp44[j]; break;
	case 46:res=byRSExp46[j]; break;
	case 48:res=byRSExp48[j]; break;
	case 50:res=byRSExp50[j]; break;
	case 52:res=byRSExp52[j]; break;
	case 54:res=byRSExp54[j]; break;
	case 56:res=byRSExp56[j]; break;
	case 58:res=byRSExp58[j]; break;
	case 60:res=byRSExp60[j]; break;
	case 62:res=byRSExp62[j]; break;
	case 64:res=byRSExp64[j]; break;
	case 66:res=byRSExp66[j]; break;
	}
	return res;
}
...
bool EncodeData(char *lpsSource)
{
	...
	nEncodeVersion = GetEncodeVersion(nVersion, lpsSource, ncLength);//计算编码数据需要的版本1-40
	//nEncodeVersion = 10;
	//LTPrintf("nEncodeVersion = %d\n",nEncodeVersion);
	if (nEncodeVersion == 0)
		return FALSE; 
	...
	//LTPrintf("m_nVersion = %d\n",m_nVersion);
	
	ncDataCodeWord = QR_VersonInfo[m_nVersion].ncDataCodeWord[m_nLevel];//得到最大输入字符数

	ncTerminater = min(4, (ncDataCodeWord * 8) - m_ncDataCodeWordBit);

	if (ncTerminater > 0)
		m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, 0, ncTerminater);//实际输入字符位数加ncTerminater
	...
	m_ncAllCodeWord = QR_VersonInfo[m_nVersion].ncAllCodeWord;//码字总数=数据码字+纠错码字
	...
	ncDataCw1 = QR_VersonInfo[m_nVersion].RS_BlockInfo1[m_nLevel].ncDataCodeWord;//指定纠错等级下的数据码字数
	...
	for (i = 0; i < ncBlock1; ++i)
	{
		...
		memmove(m_byRSWork, m_byDataCodeWord + nDataCwIndex, ncDataCw1);//将从m_byDataCodeWord + nDataCwIndex地址开始的ncDataCw1个字节复制到以m_byRSWork为首地址的空间
...
{
	int nVerGroup = nVersion >= 27 ? QR_VRESION_L : (nVersion >= 10 ? QR_VRESION_M : QR_VRESION_S);//获得字符指示位数0-2
	int i, j;
	for (i = nVerGroup; i <= QR_VRESION_L; ++i)
	{
		if (EncodeSourceData(lpsSource, ncLength, i))
		{
			...
			{
				for (j = 1; j <= 9; ++j)
				{/*m_ncDataCodeWordBit的值由输入网址的长度决定*/
...
int EncodeSourceData(char *lpsSource, int ncLength, int nVerGroup)
{
	...
	memset(m_nBlockLength, 0, sizeof(m_nBlockLength));//用来清空数组，m_nBlockLength为要进行赋值的数组，0为所要赋的值，sizeof(m_nBlockLength)为赋值数组的长度

	for (m_ncDataBlock = i = 0; i < ncLength; ++i)
	{
		unsigned char	 byMode;

		if (i < ncLength - 1 && IsKanjiData(lpsSource[i], lpsSource[i + 1]))//判断输入要生成二维码字符串的信息
		...
		if (i == 0)
			m_byBlockMode[0] = byMode;			//用来存储数据的模式，如日文还是数字等

		if (m_byBlockMode[m_ncDataBlock] != byMode)//如果前后两个数据的模式不一样
			m_byBlockMode[++m_ncDataBlock] = byMode;//将数据类型保存在已保存数据类型的数组元素的下一个

		++m_nBlockLength[m_ncDataBlock];//用来记录相邻相同数据类型的个数
	...
	++m_ncDataBlock;//相邻数据类型不同的总数包含结束符
	...
	while (nBlock < m_ncDataBlock - 1)
	{
		...
		{//如果前后2个数据块为字母和数字，可以合并成字母数字模式(字母模式包含了数字)  计算数据位流
			ncSrcBits = GetBitLength(m_byBlockMode[nBlock], m_nBlockLength[nBlock], nVerGroup) +
						GetBitLength(m_byBlockMode[nBlock + 1], m_nBlockLength[nBlock + 1], nVerGroup);//合并前长度

			ncDstBits = GetBitLength(QR_MODE_ALPHABET, m_nBlockLength[nBlock] + m_nBlockLength[nBlock + 1], nVerGroup);//合并后长度
	...
	for (i = 0; i < ncDataCodeWord ; ++i)
	{
		...
		if (lpbyRSWork[0] != 0)
		{
			...
			for (j = 0; j < ncRSCodeWord; ++j)
			{
				...
				//LTPrintf("nExpElement:%d\n",funtion(ncRSCodeWord,j));
				nExpElement = (unsigned char)(((int)(funtion(ncRSCodeWord,j)+ nExpFirst)) % 255);
...
//	memset(m_byModuleData, 0, sizeof(m_byModuleData));//清空二维数组
	write_sdram_block(0,0,200,200,0);
	/**************以下看情况是否加入清空函数***************/
...
//			m_byModuleData[i][j] = (unsigned char)((m_byModuleData[i][j] & 0x11) != 0);
			...
			write_sdram(i,j,(unsigned char)((read_sdram(i,j) & 0x11) != 0));
		}
	}
}
//设置功能性模块信息
...
//		m_byModuleData[i][m_nSymbleSize - 8] = m_byModuleData[7][m_nSymbleSize - 8 + i] = '\x20';
		...
		write_sdram(i,7, '\x20');
		write_sdram(7,i, '\x20');
		write_sdram(m_nSymbleSize - 8,i, '\x20');
		write_sdram(m_nSymbleSize - 8 + i,7, '\x20');
		write_sdram(i,m_nSymbleSize - 8, '\x20');
		write_sdram(7,m_nSymbleSize - 8 + i, '\x20');
	}
...
//		m_byModuleData[m_nSymbleSize - 8 + i][8] = m_byModuleData[8][m_nSymbleSize - 8 + i] = '\x20';
		...
		write_sdram(m_nSymbleSize - 8 + i,8,'\x20');
		write_sdram(8,m_nSymbleSize - 8 + i, '\x20');
	}
...
//		m_byModuleData[6][i] = (i % 2) == 0 ? '\x30' : '\x20';
		...
		write_sdram(i,6,(i % 2) == 0 ? '\x30' : '\x20');
		write_sdram(6,i,(i % 2) == 0 ? '\x30' : '\x20');
	}
}
//设置大回字信息
// O X X X X X X X
...
//			m_byModuleData[x + j][y + i] = (byPattern[i] & (1 << (6 - j))) ? '\x30' : '\x20';
			...
			write_sdram(x + j,y + i,(byPattern[i] & (1 << (6 - j))) ? '\x30' : '\x20');
		}
	}
}
//设置小回字信息
...
//			m_byModuleData[x + j][y + i] = (byPattern[i] & (1 << (4 - j))) ? '\x30' : '\x20';
			...
			write_sdram(x + j,y + i,(byPattern[i] & (1 << (4 - j))) ? '\x30' : '\x20');
		}
	}
}
...
//			(nVerData & (1 << (i * 3 + j))) ? '\x30' : '\x20';
			...
			write_sdram(m_nSymbleSize - 11 + j,i,(nVerData & (1 << (i * 3 + j))) ? '\x30' : '\x20');
			write_sdram(i,m_nSymbleSize - 11 + j,(nVerData & (1 << (i * 3 + j))) ? '\x30' : '\x20');
		}
	}
}
//如果你以为我们可以开始画图，你就错了。
...
//			m_byModuleData[x][y] = (m_byAllCodeWord[i] & (1 << (7 - j))) ? '\x02' : '\x00';
			...
			while (read_sdram(x,y) & 0x20);

			write_sdram(x,y,(m_byAllCodeWord[i] & (1 << (7 - j))) ? '\x02' : '\x00');
...
//				m_byModuleData[j][i] = (unsigned char)((m_byModuleData[j][i] & 0xfe) | (((m_byModuleData[j][i] & 0x02) > 1) ^ bMask));
				write_sdram(j,i,(unsigned char)((read_sdram(j,i) & 0xfe) | (((read_sdram(j,i) & 0x02) > 1) ^ bMask)));
			}
...
//		m_byModuleData[8][i] = (nFormatData & (1 << i)) ? '\x30' : '\x20';
	...
	for (i = 0; i <= 5; ++i)
		write_sdram(8,i,(nFormatData & (1 << i)) ? '\x30' : '\x20');
...
//	m_byModuleData[7][8] = (nFormatData & (1 << 8)) ? '\x30' : '\x20';
	...
	write_sdram(8,7,(nFormatData & (1 << 6)) ? '\x30' : '\x20');
	write_sdram(8,8,(nFormatData & (1 << 7)) ? '\x30' : '\x20');
	write_sdram(7,8,(nFormatData & (1 << 8)) ? '\x30' : '\x20');

//	for (i = 9; i <= 14; ++i)
//		m_byModuleData[14 - i][8] = (nFormatData & (1 << i)) ? '\x30' : '\x20';

	for (i = 9; i <= 14; ++i)
		write_sdram(14 - i,8,(nFormatData & (1 << i)) ? '\x30' : '\x20');

//	for (i = 0; i <= 7; ++i)
//		m_byModuleData[m_nSymbleSize - 1 - i][8] = (nFormatData & (1 << i)) ? '\x30' : '\x20';

	for (i = 0; i <= 7; ++i)
		write_sdram(m_nSymbleSize - 1 - i,8,(nFormatData & (1 << i)) ? '\x30' : '\x20');

//	m_byModuleData[8][m_nSymbleSize - 8] = '\x30';

	write_sdram(8,m_nSymbleSize - 8,'\x30');

//	for (i = 8; i <= 14; ++i)
//		m_byModuleData[8][m_nSymbleSize - 15 + i] = (nFormatData & (1 << i)) ? '\x30' : '\x20';

	for (i = 8; i <= 14; ++i)
		write_sdram(8,m_nSymbleSize - 15 + i,(nFormatData & (1 << i)) ? '\x30' : '\x20');
}
...
//					break;
				...
				if (((read_sdram(i,j) & 0x11) == 0) == ((read_sdram(i,k) & 0x11) == 0))
					++nCount;
				else
					break;
			}
...
//					break;
				...
				if (((read_sdram(j,i) & 0x11) == 0) == ((read_sdram(k,i) & 0x11) == 0))
					++nCount;
				else
					break;
			}
	...
	for (i = 0; i < m_nSymbleSize - 1; ++i)
	{
		...
		{
			if ((((read_sdram(i,j) & 0x11) == 0) == ((read_sdram(i + 1,j)		& 0x11) == 0)) &&
				(((read_sdram(i,j) & 0x11) == 0) == ((read_sdram(i,j + 1) & 0x11) == 0)) &&
				(((read_sdram(i,j) & 0x11) == 0) == ((read_sdram(i + 1,j + 1) & 0x11) == 0)))
			{
				nPenalty += 3;
			}
...
//			}
	...
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		...
		{
			if (((j == 0) ||				 (! (read_sdram(i,j - 1) & 0x11))) && // � ��� �����
											 (   read_sdram(i,j)     & 0x11)   && // � - 1
											 (! (read_sdram(i,j + 1) & 0x11))  && // � - 1
											 (   read_sdram(i,j + 2) & 0x11)   && // � �
											 (   read_sdram(i,j + 3) & 0x11)   && // � �3
											 (   read_sdram(i,j + 4) & 0x11)   && // � �
											 (! (read_sdram(i,j + 5) & 0x11))  && // � - 1
											 (   read_sdram(i,j + 6) & 0x11)   && // � - 1
				((j == m_nSymbleSize - 7) || (! (read_sdram(i,j + 7) & 0x11))))   // � ��� �����
			{
				if (((j < 2 || ! (read_sdram(i,j - 2) & 0x11)) &&
					 (j < 3 || ! (read_sdram(i,j - 3) & 0x11)) &&
					 (j < 4 || ! (read_sdram(i,j - 4) & 0x11))) ||
					((j >= m_nSymbleSize - 8  || ! (read_sdram(i,j + 8)  & 0x11)) &&
					 (j >= m_nSymbleSize - 9  || ! (read_sdram(i,j + 9)  & 0x11)) &&
					 (j >= m_nSymbleSize - 10 || ! (read_sdram(i,j + 10) & 0x11))))
				{
					nPenalty += 40;
				}
			}
...
//			}
		...
		if (((j == 0) ||				 (! (read_sdram(j - 1,i) & 0x11))) && // � ��� �����
											 (   read_sdram(j,i) & 0x11)   && // � - 1
											 (! (read_sdram(j + 1,i) & 0x11))  && // � - 1
											 (   read_sdram(j + 2,i) & 0x11)   && // � �
											 (   read_sdram(j + 3,i) & 0x11)   && // � �3
											 (   read_sdram(j + 4,i) & 0x11)   && // � �
											 (! (read_sdram(j + 5,i) & 0x11))  && // � - 1
											 (   read_sdram(j + 6,i) & 0x11)   && // � - 1
				((j == m_nSymbleSize - 7) || (! (read_sdram(j + 7,i)& 0x11))))   // � ��� �����
			{
				if (((j < 2 || ! (read_sdram(j - 2,i) & 0x11)) &&
					 (j < 3 || ! (read_sdram(j - 3,i) & 0x11)) &&
					 (j < 4 || ! (read_sdram(j - 4,i) & 0x11))) ||
					((j >= m_nSymbleSize - 8  || ! (read_sdram(j + 8,i)  & 0x11)) &&
					 (j >= m_nSymbleSize - 9  || ! (read_sdram(j + 9,i)  & 0x11)) &&
					 (j >= m_nSymbleSize - 10 || ! (read_sdram(j + 10,i) & 0x11))))
				{
					nPenalty += 40;
				}
			}
...
//			}
			...
			if(read_sdram(j,k) == 1 && read_sdram(j,k + 1) == 1)
			{
				print_buff[x] = 0xFF;
				x++;
			}
			else if(read_sdram(j,k) == 1 && read_sdram(j,k + 1) == 0)
			{
				print_buff[x] = 0xF0;
				x++;
			}
			else if(read_sdram(j,k) == 0 && read_sdram(j,k + 1) == 1)
			{
				print_buff[x] = 0x0F;
				x++;
			}
...
void write_sdram(unsigned short x,unsigned short y,UINT16 data)
{

}


UINT16 read_sdram(unsigned short x,unsigned short y)
{

}

void write_sdram_block(unsigned short x,unsigned short y,unsigned short w,unsigned short h,UINT16 data)
{
